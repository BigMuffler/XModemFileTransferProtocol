...
void SenderX::genBlk(blkT blkBuf)
{
	...
	blkBuf [1] = blkNum;
	blkBuf [2] = 255 - blkNum;
	int padding = 128 - bytesRd;
	if (bytesRd < 128)
	{
	for (int i = 0; i<padding; i++)
	  {
	       blkBuf[3+i+bytesRd]= CTRL_Z;
	  };
	};
	...
	while (totalBytes > 255) //check sum
	{
	    totalBytes-=256;
	};

	blkBuf [131] = totalBytes;
//
